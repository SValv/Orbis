--- 
title: "<center>Orbis Astea</center>"
date: "<center>Version vom: `r format(Sys.Date(),'%d.%m.%Y')`</center>"
author: "<center>DM-App Version: 3.4531</center>"
...
file.copy(paste0(pfad,"Container/",c("CCSheetBlank.xlsx","CCrand.xlsx")), 
...
RegionenLabels= sprintf(
  "<strong>%s</strong><br/>Hauptstadt: %s<br>Herrscher: %s<br>Bevölkerungsdichte: %s pro Km<small><sup>2</sup></small>",
...
getFeiertag = function(wherees){
  if (wheres %in% RegionPolygonsData$Name){
    df = Feiertagdf %>% filter(wherees %in% Relevanz)
  } else if(wheres %in% RegionPolygonsData$Reich){
    joindf = RegionPolygonsData %>% select(Reich,Name) %>% filter(wherees == Reich)
    df= Feiertagdf %>% filter(any(joindf$Name %in% Relevanz))
  }
  df= knitr::kable(df)
  return(df)
}
