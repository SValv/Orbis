...
Das "Absaugen" aus den Leichen noch nicht Erwachter macht die Leichen 
...
Die erste Möglichkeit einen Befehl zu äußern liegt vor sobald der Spieler Charakter initiative hat. Die Aktion des Konstruktes erfolgt zeitlich parallel mit der des Spielers außer der Befehl tritt erst zeit versetzt in kraft. Dem Konstrukt stehen wie jedem Char zwei Aktionen zur Verfügung. Nach der ersten Runde folgt das Konstrukt der normalen Initativtabelle.
...
Ein Konstrukt besteht aus Corpus, Kern und Batterie. Alle müssen für ein funktionierendes Konstrukt vorhanden sein können jedoch auch beliebig zwischen ihnen getauscht werden.
...
Der Corpus gibt die Verfügbaren Attribute.

#### Gliedmaßen / Schwachpunkt

Konstrukt Teile: Jedes Konstrukt bestehlt aus 6 angreifbaren Teilen seinen 4 Gliedmaßen einer Kernabdeckung und dem Kern selbst. Der Kern für sich ist nahezu ungepanzert, kann jedoch erst angegriffen werden *nachdem* die Kernabdeckung zerstört wurde. Der Kern ersetzt beim gezielten Schussangriff den Kopf und die Kernabdeckung den Torso.

#### Kern

Der Konstruktkern stellt sowohl das Herz als auch das Hirn eines jeden Konstruktsdar
Das Level des Kern wird durch das Anwenden von Essenzen in der nebenbei Downtime erhöht
...
#### Addons / Waffen / Upgrades

Begrenzte Anzahl auf 5 pro Konstrukt

Addons sind selbst durch Handwerkherstellbar. Die Attribut erhöhung sowie der Waffenschaden sind dabei Abhängig vom verwendeten Material Tier.
...
Transport :75, 125, 200, 300 KG zusätzliich

Panzermodus 100%  Block Chance; Tiers erhöhen den Schildblockwert

Wachmodus = Standartmäßig vorhanden benötigt dennoch Platz, läuft über Perception
...
Gehen, Wachmodus, Panzermodus aktivieren
...
Dem Besitzer des Konstrutks ist es möglich kurzzeitig die Kontrolle über ein Konstukt auch aus der Entfernung zu übernehmen, dies äußert sich in einer zusätzlichen Aktion für das Konstrukt. Diese Aktion hängt von einem Vehicel Check ab dessen schwierigkeit auf dem Kern und der Größe des Konstrukts basiert, je stärker das Konstrukt desto schwieriger die Kontrolle. Bei Erfolg wird die Main Action des Characters aufgebraucht, bei Misserfolg die Singleaction da weniger Zeit verbraucht wird. 

Die Schwierigkeit des Checks wird im Konstruktprofil automatisch berechnet.
Die Schwierigkeit kann durch einen Pilotensitz im Konstrukt halbiert werden.
