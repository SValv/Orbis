...
In Orbis Astea wird - wie in jedem anderen Pen and Paper Rpg - gewürfelt. Bei uns wird dies durch ein Programm übernommen, daher gibt es hier nichts selbst zu rechnen. Trotzdem ist es sehr relevant zu wissen wie welche Probe gewürfelt wird. Für jede Probe gibt es einen sogenannten [Fähigkeiten](#Fähigkeiten), der auf die Aktion passt - bspw. ["Bewaffnet"](#Bewaffnet) für das Zuschlagen mit einer Waffe oder ["Schleichen"](#Schleichen) für das Heranschleichen an eine Situation. Die passende Probe zur Aktion wird dann mit der Fähigkeit und dem verbundenen Attribut geworfen - für das Beispiel Schleichen wäre dieses "Agilität".
...
Da jeder Trank toxisches Potential hat, wird immer, nachdem ein Trank konsumiert wird, ein Toxi-Save gegen den Threshold aus allen kumulierten Trankstufen der genommenen Tränke gewürfelt. Sollte man diesen nicht schaffen fällt man in [Ohnmacht](#Ohnmacht).
...
Am Anfang des Kampfes wirft jeder seine - diese Berechnet sich mit $(Instinct + Agility)/2$. Die Werte werden nach größe geordnet und der mit dem höchsten Wert darf angreifen, dann der mit dem zweithöchsten und so weiter. Außerdem ist di eGeschwindigkeit von Waffen ein Faktor, der stark auf die Initiative einwirkt. Manchmal legt der DM die Initiavit-Reihenfolge auch sinngemäß fest (bspw. bei Schleichangriffen). 

Am Ende jeder Runde wird eine neue Initiativreihenfolge festgelgt.
...
So gut wie jede Waffe hat einen Bereichsangriff - also einen Angriff, der AoE-Schaden machen kann. Bei Fernwaffen müssen Ziele dafür hintereinander stehen, bei Nahkamfangriffen realistisch in der Nähe. Ein AoE-Angriff macht den normalen Angriffswert mal dem Durchschlagungswert. Ein AoE-Angriff kann mit einer guten Parade oder einem gutem Block abgewehrt und abgebrochen werden.

Am geeignetsten für AoE-Angriffe sind zweihändige Großwaffen.
...
Wurfwaffen funktionieren für gewöhnlich genauso. Im Notfall kann man sie als normale Nahkampfwaffen verwenden, allerdings sollte man dies nur im Notfall tun. Ansonsten werden Wurfwaffen für eine single Action geworfen. Sie haben eine Chance gegnerische Aktionen zu [unterbrechen](#interrupt), sie haben allerdings auch bei jeder Verwendung eine Chance, kaputt zu gehen. Eine [Liste von Wurfwaffen ist hier](#Wurfwaffen) einzusehen. Falls man sich die Single Action aufhebt, kann man den Gegner auch in der Aktion unterbrechen.
...
|    Länge    |                           Dauer                           | Preise               |
|:--------------:|:---------------------------------------:|----------------|
| Kurze Pause |                        3-4 Stunden                        | 1 Vorrat pro Person  |
| Lange Pause |                        7-8 Stunden                        | 2 Vorräte pro Person |
|  Downtime   | Zeit zwischen 2 Sessions <br>(üblicherweise mehrere Tage) | 3 Vorräte pro Person |

Jede Option kostet Vorräte die in den Örtlichen Handelskontoren erworben, oder in der Downtime erjagt werden können. Alternativ können Vorräte auch durch 1KG an Lebensmitteln ersetzt werden. Sollten keine Vorräte eingesetzt werden ist eine Erholung nicht möglich.
...
Neben den Downtime Aktivitäten mit denen ausschließlich monetäre Güter erworben werden können existieren auch weitere wie die Suche nach Alchemistischen Kräutern, wie an anderer STelle erläutert. Jedoch ist es auch möglich auf die Jagt zu gehen. Mit dieser lassen sich die Vorräte welche für Rast und Downtime notwendig sind eigenhändig erjagen, einfluss auf das Ergebnis hat sowohl die Umgebung sei es nun in der Wildniss, die umgebung eines Dorfes oder gar eine Stadt, sowie die Fähigkeiten Wahrnehmung und schleichen sowie deren jeweiligen Attribute.
...
Wie beim Training muss man zuerst spezifizieren was man studieren will - also in welchem Bereich man sich weiterbilden will. Studieren ist ein langwieriger Prozess. Man verbraucht beim studieren jedes Mal eine volle Downtime.

Beim Studieren wird eine Intelligenz-Probe gegen einen Threshold geworfen. Ist diese erfolgreich, ist der Wissensskill auf der jeweiligen Stufe gelernt. Der Threshold ist abhängig von der Hohe der Wissensstufe, die erlernt werden soll.

| Niveau/Stufe der Sprache oder des Wissens: | Threshold Probe: |          Wissensniveau           |                Sprachniveau                |
|:------------------:|:--------------:|:--------------:|:------------------:|
|                  Stufe 0                   |        0         |      Kein Wissen im Bereich      |           Keine Sprachfähigkeit            |
|                  Stufe 1                   |        2         |           Grundwissen            |      sehr einfache Gespräche möglich       |
|                  Stufe 2                   |        3         | Belesen & Erfahren im Fachgebiet | Verständigung umständlich aber vollständig |
|                  Stufe 3                   |        5         |     Ausgewiesener Fachmensch     |   Gesetzestext- und Muttersprachenniveau   |
...
```{r}
dfe=read_xlsx(bookdownallgdatapfad,sheet="Beruf Einkommen ")

dfe = dfe[,c(1,5,6)]
colnames(dfe) = c("Beruf", "Monatseinkommen", "Jahreseinkommen")


dfe %>% arrange(Jahreseinkommen) %>% 
